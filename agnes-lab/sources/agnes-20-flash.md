...
# Agnes 2.0 Flash (Deprecated)

> A deprecated Agnes text model. Use Agnes 2.5 Flash for new integrations.

<Info>
  Agnes 2.0 Flash is deprecated and is no longer recommended for new API integrations. Existing callers should migrate to `agnes-2.5-flash`.
</Info>

<Warning>
  `agnes-2.0-flash` is deprecated. This page is retained for historical endpoint and parameter reference only. Change the request `model` to `agnes-2.5-flash`.
</Warning>
...
<CardGroup cols={2}>
  ...
  <Card title="Model Status" icon="triangle-exclamation">
    Deprecated; migrate to `agnes-2.5-flash`.
  </Card>
...
The following specifications and prices are retained as historical records for this deprecated model.
...
<Check>
  Stop using the deprecated `agnes-2.0-flash` model and migrate to `agnes-2.5-flash`.
</Check>
